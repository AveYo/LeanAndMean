...
write-host @'
  ...
  FixNetworkBufferbloat - AveYo, 2025.09.10
...
foreach ($a in $do.Values) { if ("$(split-path $env:0 -leaf) $env:1" -like "*$a*") {$cl = $a} }
...
$UF = (" ",[char]0x2713)[(gp "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" "U fix" -ea 0) -ne $null]
$DF = (" ",[char]0x2713)[(gp "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" "D fix" -ea 0) -ne $null]
...
$ps = {
  ...
  ##  current state
  $UF = ($false,$true)[(gp "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" "U fix" -ea 0) -ne $null]
  $DF = ($false,$true)[(gp "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" "D fix" -ea 0) -ne $null]
  ...
  if ($do -eq 'upload' -or $do -eq 'both') {
    $RWSCALING = 'Normal'; $NONSACK = 'Enabled'; if ($do -eq 'both') { $RWSCALING = 'Disabled' }
    write-host " Upload QoS ON" -fore Yellow; . {
      if ($do -ne 'both') {
        netsh int tcp set global autotuninglevel=Normal
        rp "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" "D fix" -force -ea 0
      }
      ni "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" -ea 0
      sp "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" "U fix" 1 -type string -force -ea 0
      ...
      #sp "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Psched" TimerResolution 1 -type dword -force -ea 0
      sp "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Psched" NonBestEffortLimit 20 -type dword -force -ea 0 ## 4g experiment: 20
      ...
      $rq = "Remove-NetQosPolicy -PolicyStore ActiveStore -name * -Confirm:`$false -ea 0"
      $tq = "New-NetQosPolicy Bufferbloat_throttle -PolicyStore ActiveStore -NetworkProfile 2 -IPProtocol TCP " +
            "-Precedence 254 -DSCPAction 10 -MinBandwidthWeightAction 2"  ## 4g experiment: 2
      $pq = "New-NetQosPolicy Bufferbloat_priority -PolicyStore ActiveStore -NetworkProfile 2 -Default " +
            "-Precedence 253 -DSCPAction 40 -MinBandwidthWeightAction 98" ## 4g experiment: 98
      powershell -nop -c "$rq;$tq;$pq"
      $sa = New-ScheduledTaskAction -Execute powershell -Argument "-nop -c $rq;$tq;$pq"; $st = New-ScheduledTaskTrigger -AtStartup
  ...
  if ($do -eq 'download' -or $do -eq 'both') {
    $RWSCALING = 'Disabled'; $NONSACK = 'Disabled'; if ($do -eq 'both') { $NONSACK = 'Enabled' }
    write-host " Download Autotuning OFF" -fore Yellow; . {
      ...
      ni "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" -ea 0
      sp "HKLM:\SYSTEM\CurrentControlSet\Services\Tcpip\QoS" "D fix" 1 -type string -force -ea 0
  ...
  if ($do -ne 'reset') {
    ...
    " Netsh tweaks"; . {
      netsh winsock set autotuning off                                   # Winsock send autotuning
      ...
      netsh int tcp set supplemental internet congestionprovider=ctcp    # Controls the congestion provider. cubic newreno dctcp
      ...
      netsh int tcp set supplemental internet rack=disabled              # Controls whether RACK time based recovery is enabled.
      ...
      netsh int tcp set global timestamps=allowed             # Enable/disable RFC 1323 timestamps. * allowed enabled
  ...
  #. { gpupdate /Target:Computer /force } 2>'' 1>''
